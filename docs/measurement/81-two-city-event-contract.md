...
**Funnel step-over-step**, same denominator pattern as #66: for each pair of
adjacent steps in `location_selected → home_viewed → restaurant_opened →
cart_viewed → checkout_viewed → order_placed`, `count(distinct visitor_id)`
at the later step ÷ the earlier step's.
